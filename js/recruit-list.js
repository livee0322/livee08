...
(() => {
  ...
  function toast(msg){
    ...
    if(!t){ t = document.createElement('div'); t.id='lvToast';
      t.style.cssText='position:fixed;left:50%;bottom:90px;transform:translateX(-50%);background:rgba(0,0,0,.8);color:#fff;padding:10px 14px;border-radius:999px;z-index:9999;font-size:13px';
  ...
  function mapStatus(s){
    ...
    if(s==='closed') return {key:'closed', label:'마감', cls:'closed'};
    if(s==='draft') return {key:'draft', label:'임시저장', cls:'draft'};
    return {key:'open', label:'모집중', cls:'open'};
  }
      ...
      <article class="rl-card">
        ...
        <div class="rl-body">
          ...
          <div class="rl-meta">
            ...
            ${it.closeAt?`<span>마감 <b style="color:var(--pri)">${fmtDate(it.closeAt)}</b></span>`:''}
            <span>출연료 ${pay}</span>
            ${it.views!=null?`<span>조회 ${it.views}</span>`:''}
            ${it.applies!=null?`<span>지원 ${it.applies}</span>`:''}
          </div>
        </div>
  ...
  function bindUI(){
    ...
    $('#rlList').addEventListener('click', e=>{
      ...
      if(!canManage){
        toast('브랜드만 이용 가능한 기능입니다.');
        return;
      }
      if(act==='edit'){ location.href = `recruit-new.html?id=${encodeURIComponent(id)}`; }
      else if(act==='dup'){ toast('복제 준비중'); /* POST /:id/duplicate */ }
      else if(act==='close'){ toast('마감 처리 준비중'); /* PATCH status=closed */ }
      else if(act==='del'){ toast('삭제 준비중'); /* DELETE */ }
    });
  }
