...
(function(w){
  ...
  function renderHero(){
    ...
    const heroBg = D?.hero?.image ? `background-image:linear-gradient(to top, rgba(0,0,0,.35), rgba(0,0,0,.08)),url('${D.hero.image}')` : '';
    root.innerHTML = `
      <div class="media" style="${heroBg}"></div>
      <div class="body">
        <div class="row">
  ...
  function renderSlots(){
    ...
    const slots = (D?.availability?.timeslots||[]).filter(Boolean);
    if(!calState.pickedDate || slots.length===0){
      card.hidden = true; return;
    }
    $('#bhPickedDate').textContent = calState.pickedDate;
    const wrap = $('#bhSlots'); wrap.innerHTML = slots.map(t=>{
      const sel = (calState.pickedTime===t)?'is-sel':'';
      return `<button type="button" class="bh-chip2 ${sel}" data-t="${t}">${t}</button>`;
    }).join('');
    card.hidden=false;
    const cta = $('#bhOpenReserve'); if(cta) cta.disabled = !calState.pickedTime;
  ...
  function renderPricing(){
    ...
    const html = (D?.pricing||[]).map(p=>`
      <article class="bh-plan">
        ...
        <div class="name">${p.name||''} ${p.duration?`· <span style="color:#64748b">${p.duration}</span>`:''}</div>
  ...
  function renderGallery(){
    ...
    root.innerHTML = galState.list.map((src,i)=>`<img src="${src}" alt="gallery-${i}" data-i="${i}" loading="lazy">`).join('') || '<div style="color:#64748b">이미지가 없습니다.</div>';
  }
  ...
  function openLightbox(list, idx){
    ...
    wrap.classList.add('show'); wrap.setAttribute('tabindex','-1'); wrap.focus();
  }
  ...
  function openModal(kind, payload={}){
    ...
    if(kind==='inquiry'){
      ...
      on($('#inqSubmit',wrap),'click',()=>{
        const name=$('#inqName').value.trim(), phone