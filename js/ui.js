...
(() => {
    ...
    window.UI = {
      ...
      toast(msg) {
        ...
        if (!t) {
          ...
          t.id = '__toast';
          t.style.cssText =
            'position:fixed;left:50%;bottom:76px;transform:translateX(-50%);background:#111827;color:#fff;padding:10px 14px;border-radius:12px;font-weight:800;z-index:1200;transition:opacity .2s';
          document.body.appendChild(t);
        }
        t.textContent = msg;
        t.style.opacity = '1';
        clearTimeout(t.__to);
        t.__to = setTimeout(() => (t.style.opacity = '0'), 1300);
      },
      qs() {
  ...
  ready(() => {
    ...
    // 페이지 본문이 탭과 겹치지 않도록 안전여백 보장
    const root = document.querySelector('main') || document.body;
    const pb = parseInt(getComputedStyle(root).paddingBottom || '0', 10);
    root.style.paddingBottom =
      'calc(' + Math.max(pb, 88) + 'px + env(safe-area-inset-bottom))';
  });
})();