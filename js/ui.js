...
(() => {
  // utils
  const getToken = () => localStorage.getItem('livee_token');
  const getRole  = () => localStorage.getItem('liveeRole'); // 'brand' | 'showhost' | null
    ...
    appbar.innerHTML = `
      <div class="lv-appbar">
        ...
        <div class="lv-actions">
          ...
          <a class="lv-action" href="${getToken() ? 'login.html?logout=1' : 'login.html'}" aria-label="${getToken() ? '로그아웃' : '로그인'}">
            <i class="${getToken() ? 'ri-logout-box-r-line' : 'ri-login-box-line'}"></i>
          </a>
        </div>
      </div>`;
  }
