...
(() => {
  ...
  const validDeadline=()=>{
    ...
    return deadline.value<=shootDate.value; // YYYY-MM-DD 문자열 비교 OK
  };
  ...
  fileEl?.addEventListener('change', async e=>{
    ...
    try{
      ...
      previewEl.src=thumb;
      previewEl.dataset.cover=cover;
      previewEl.dataset.thumb=thumb;
      say('이미지 업로드 완료', true);
    }catch(err){
  ...
  form?.addEventListener('submit', async ev=>{
    ...
    const brandName=(brandNameEl?.value||'').trim(); // 상/하위 모두 brandName로 보냄
    ...
    const payload={
      ...
      recruit:{
        recruitType:"product",
        brandName,                       // 하위(recruit.brandName)도 기록
