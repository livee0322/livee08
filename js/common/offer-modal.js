/* js/common/offer-modal.js — v1.0.0
   - data-offer 클릭 → 모달 표시
   - 전송 성공 → 알림 + 모달 닫기 (수신자는 마이페이지 '받은 제안'에서 확인)
*/
(function () {
    ...
    el.innerHTML = `
      <div class="lv-sheet">
        ...
        <div class="lv-sheet__bd">
          ...
          <label class="lv-lb">메시지 (선택)</label>
  ...
  function openOfferModal({ portfolioId, recruitId }){
    ...
    // 전송
    send.onclick = async ()=>{
      if(!TOKEN){ alert('로그인이 필요합니다.'); location.href = (window.LIVEE_CONFIG?.BASE_PATH||'') + '/login.html'; return; }
      send.disabled = true;
      try{
        const payload = { toPortfolioId: portfolioId, message: msg.value.trim() };
        if (recruitId) payload.recruitId = recruitId;
  ...
  // ── 트리거: 아무 곳에서나 data-offer 버튼으로 사용
  ...
  // ── 간단 스타일(없으면 기본 모달이 너무 밋밋하므로 최소치만)
  const css = `
  ...
  .lv-sheet{width:min(520px,100%);max-height:85vh;overflow:auto;background:#fff;border-radius:14px 14px 0 0;padding:14px}
