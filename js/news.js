/* News page — modal write + publish now */
(() => {
  ...
  const FALLBACK_IMG = CFG.placeholderThumb || 'assets/default.jpg';

  // ===== mount common UI via ui.js =====
  try{
    window.LIVEE_UI?.mountHeader?.({ title:'뉴스', right:[{ id:'openWrite', label:'작성하기', kind:'primary' }] });
    window.LIVEE_UI?.mountTopTabs?.({ active:'news' });
  ...
  const pickThumb = n => n.thumbnailUrl || n.imageUrl || n.thumbUrl || n.thumb || n.coverImageUrl || '';
  const parseList = j => (Array.isArray(j) && j) || j.items || j.data?.items || [];

  const relTime = iso => {
    if(!iso) return '';
    const d=new Date(iso), diff=Math.max(0,(Date.now()-d)/1000);
    const m=Math.floor(diff/60), h=Math.floor(m/60), dd=Math.floor(h/24);
    return dd?`${dd}일 전`:h?`${h}시간 전`:m?`${m}분 전`:'방금 전';
  };
  ...
  // 트리거: 앱바 버튼/ FAB (기본 동작 방지)
  document.body.addEventListener('click', (e)=>{
    if(e.target.matches('#openWrite')){ e.preventDefault(); openModal(); }
    if(e.target.matches('#fabWrite')) { e.preventDefault(); openModal(); }
  ...
  async function getSignature(){
    ...
    const r = await fetch(`${API_BASE}/uploads/signature`, { headers });
    const j = await r.json().catch(()=>({}));
    if(!r.ok || j.ok===false) throw new Error(j.message || `HTTP_${r.status}`);
    return j.data || j;
  }
