...
(() => {
  ...
  const pickThumb=o=> o?.mainThumbnailUrl||o?.thumbnailUrl||(Array.isArray(o?.subThumbnails)&&o.subThumbnails[0])||(Array.isArray(o?.subImages)&&o.subImages[0])||o?.coverImageUrl||o?.imageUrl||o?.thumbUrl||FALLBACK_IMG;
  ...
  const pickBrand=o=>{
    const c=[o?.brandName,o?.brandname,o?.recruit?.brandName,o?.recruit?.brandname,o?.brand?.brandName,o?.brand?.name,o?.brand,o?.owner?.brandName,o?.owner?.name,o?.user?.companyName,o?.user?.brandName];
    const v=c.find(v=>v!=null && String(v).trim()!=='');
    return v?String(v).trim():'브랜드';
  };
  const pickPay=o=>{
    const rc=[o?.fee,o?.recruit?.pay,o?.pay,o?.recruit?.payment,o?.payment,o?.recruit?.payAmount,o?.payAmount,o?.recruit?.price,o?.price];
    const raw=rc.find(v=>v!=null && String(v).trim()!=='');
    const nc=[o?.feeNegotiable,o?.recruit?.payNegotiable,o?.payNegotiable,o?.recruit?.negotiable,o?.negotiable];
    const payNegotiable=truthy(nc.find(v=>v!==undefined) ?? false);
  ...
  async function fetchRecruits(){
    try{
      ...
      return arr.map((c,i)=>{ const {pay,payNegotiable}=pickPay(c);
        return { id:c.id||c._id||`${i}`, title:c.title||c.recruit?.title||'제목 없음', thumb:pickThumb(c),
          closeAt:c.closeAt||c.recruit?.closeAt||c.deadline||c.recruit?.deadline, brandName:pickBrand(c), pay, payNegotiable };
      });
    }catch{ return []; }
  }
  async function fetchPortfolios(){
    try{
      ...
      return arr.map((p,i)=>({ id:p.id||p._id||`${i}`, nickname:text(p.nickname||p.displayName||p.name||'쇼호스트'), headline:text(p.headline||''), thumb:pickThumb(p) }));
    }catch{ return []; }
  }
  async function fetchNews(fallback=[]){
    try{
      const arr=parseItems(await getJSON(`${API_BASE}${EP_NEWS.startsWith('/')?EP_NEWS:'/'+EP_NEWS}`));
      if(arr.length){ return arr.map((n,i)=>({ id:n.id||n._id||`${i}`, title:text(n.title||n.headline||'뉴스'), date:n.publishedAt||n.createdAt||n.updatedAt, summary:text(n.summary||n.excerpt||'') })); }
    }catch{}
  ...
  function renderHero(el){
    ...
    const m=el.querySelector('.hero-media');
    if(m) m.style.backgroundImage=`linear-gradient(to top, rgba(0,0,0,.35), rgba(0,0,0,.08)), url('${heroSrc}')`;
    const nav=document.querySelector('.hero-nav'); if(nav) nav.style.display='none';
  }

  const tplLineupList=items=>!items.length?`
    <div class="ed-grid"><article class="card-ed"><div class="card-ed__body">
      <div class="card-ed__title">등록된 라이브가 없습니다</div><div class="card-ed__meta">브랜드 공고를 등록해보세요</div>
    </div></article></div>`:`
    <div class="ed-grid">
      ${items.map(r=>`
        <article class="card-ed" onclick="location.href='recruit-detail.html?id=${encodeURIComponent(r.id)}'">
          ...
          <div class="card-ed__body">
            <div class="card-ed__eyebrow">${r.brandName}</div>
            <div class="card-ed__title">${r.title}</div>
            <div class="card-ed__meta">마감 ${r.closeAt?fmtDate(r.closeAt):'미정'} · ${r.payNegotiable?'협의':(r.pay?`${money(r.pay)}원`:'출연료 미정')}</div>
          </div>
        </article>`).join('')}
    </div>`;

  const tplRecruitHScroll=items=>!items.length?`
    <div class="hscroll"><article class="card-mini" aria-disabled="true">
      <div class="mini-thumb" style="background:#f3f4f6"></div><div><div class="mini-title">공고가 없습니다</div><div class="mini-meta">새 공고를 등록해보세요</div></div>
    </article></div>`:`
    <div class="hscroll">
      ${items.map(r=>`
        <article class="card-mini" onclick="location.href='recruit-detail.html?id=${encodeURIComponent(r.id)}'">
          ...
          <div>
            ...
            <div class="mini-meta">마감 ${r.closeAt?fmtDate(r.closeAt):'미정'} · ${r.payNegotiable?'협의':(r.pay?`${money(r.pay)}원`:'미정')}</div>
          </div>
  ...
  const tplPortfolios=items=>!items.length?`
    <div class="ed-grid"><article class="card-ed"><div class="card-ed__body">
      <div class="card-ed__title">포트폴리오가 없습니다</div><div class="card-ed__meta">첫 포트폴리오를 등록해보세요</div>
    </div></article></div>`:`
    <div class="pf-hlist">
  ...
  const tplCtaBanner=()=>`
    <div class="cta-banner" role="region" aria-label="상담 배너">
      <div class="cta-copy"><div class="cta-kicker">무료 상담</div><div class="cta-title">지금 바로 라이브 커머스 시작해보세요</div><div class="cta-sub">기획 · 섭외 · 계약 · 결제까지 도와드립니다</div></div>
      <div class="cta-actions"><a class="btn" href="recruit-new.html"><i class="ri-megaphone-line"></i> 공고 올리기</a><a class="btn" href="help.html#contact"><i class="ri-chat-1-line"></i> 빠른 문의</a></div>
    </div>`;

  function sectionBlock(title, moreHref, innerHTML, key){
    return `<div class="section" data-sec="${key||''}"><div class="section-head"><h2>${title}</h2><a class="more" href="${moreHref}">더보기</a></div>${innerHTML}</div>`;
  }

  async function render(){
    const root=$('#home'); const heroRoot=$('#hero');
