...
(() => {
      ...
      items.map(r=>`
        <article class="card-mini">
          ...
          <div class="mini-body">
            ...
            <div class="lv-meta">마감 ${r.closeAt ? fmtDate(r.closeAt) : '미정'} · 출연료 ${r.payNegotiable ? '협의 가능' : (r.pay ? `${money(r.pay)}원` : '미정')}</div>
          </div>
        </article>`).join('')
    }</div>`;
  }
  ...
  function injectOnce(){
    ...
    const css=document.createElement('style'); css.id='_home_inject';
    css.textContent=`
