/* Portfolio Create — FINAL v2.11
   - API:   /api/v1/portfolio-test
   - Create/Update with unified schema
   - Publish requires: nickname, headline, mainThumbnailUrl
   - bio length: 자유(최대 5000) — 최소 길이 제한 없음
...
(() => {
  ...
  const safeBind = (btn, input)=>{
    ...
    bind(btn,'pointerdown',e=>{
      if(e.pointerType==='touch'){ e.preventDefault(); input.click(); }
    }, {passive:false});
  };
  ...
  function setPreview(kind, url){
    if(!url) return;
    if(kind==='main' && mainPrev){
      mainPrev.src = url; mainPrev.style.display=''; mainTrigger?.classList?.remove('is-empty');
    }
    if(kind==='cover' && coverPrev){
      coverPrev.src = url; coverPrev.style.display=''; coverTrigger?.classList?.remove('is-empty');
    }
  }
  ...
  function drawSubs(){
    ...
    subsGrid.innerHTML = `
      ${items}
      <button type="button" class="pf-addThumb" id="subsTrigger2" aria-label="서브 이미지 추가">+</button>`;
  ...
  function collectPayload(status){
    ...
    // 통일 스키마 — !!! name/ displayName 같은 구버전 필드는 더 이상 전송하지 않음
    return {
      type: 'portfolio',
      status,
  ...
  async function loadIfEdit(){
    ...
    try{
      ...
      bio && (bio.value = d.bio || '');
      realName && (realName.value = d.realName || '');
      realNamePublic && (realNamePublic.checked = !!d.realNamePublic);
      age && (age.value = d.age || '');
  ...
  (async ()=>{
    ...
    if(!hasRole(me, 'showhost')){
      ...
      const guard = document.getElementById('pfGuard'); guard?.classList.add('show');
      return;
    }
    await loadIfEdit();
  })();
